...
;; Mood tracker
(defun my/mood-agenda (&rest _ignore)
  (let ((inhibit-read-only t)   ; allow writing inside *Org Agenda*
        entries)

    (with-current-buffer
        (find-file-noselect (concat org-directory "/mood.org"))
      (org-map-entries
       (lambda ()
         (let* ((body (buffer-substring-no-properties
                       (point) (org-entry-end-position)))
                (date  (and (string-match ":CREATED: \\[\\([0-9-]+\\)" body)
                            (match-string 1 body)))
                (score (and (string-match "Score: \\([0-9]+\\)" body)
                            (match-string 1 body)))
                (sleep (and (string-match "Sleep: \\([0-9.]+\\)" body)
                            (match-string 1 body)))
                (notes (and (string-match "Notes: \\(.+\\)" body)
                            (match-string 1 body))))
           (when score
             (push (list date score sleep notes) entries))))
       "LEVEL=4"))

    (setq entries (nreverse entries))

    (let* ((scores (mapcar (lambda (e) (string-to-number (cadr e))) entries))
           (avg (if scores (/ (float (apply #'+ scores)) (length scores)) 0)))

      (insert (format "📊 Mood Report   avg: %.1f/10   entries: %d\n"
                      avg (length scores)))
      (insert (make-string 70 ?─) "\n")

      (dolist (e entries)
        (let* ((score (string-to-number (cadr e)))
               (bar (make-string score ?█))
               (pad (make-string (- 10 score) ?░)))
          (insert (format "  %-12s  %s%s %s/10  💤 %sh  %s\n"
                          (or (car e) "?")
                          bar pad score
                          (or (caddr e) "?")
                          (or (cadddr e) "")))))
